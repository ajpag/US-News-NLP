...
all_news_raw <- "https://raw.githubusercontent.com/ajpag/US-News-NLP/main/data/news_all.csv?token=ATPY7SVPUULWBKBCM5Q2SFTAR76MI"
...
#Re-code zero values as NA to distinguish from mean = 0
...
#Code NA sentiment in words as zero understanding they have no sentiment related to that word
new_features$Biden_sentiment[new_features$Biden_sentiment == "NaN"] <- 0
new_features$Trump_sentiment[new_features$Trump_sentiment == "NaN"] <- 0
new_features$stock_market_sentiment[new_features$stock_market_sentiment == "NaN"] <- 0
new_features$financial_sentiment[new_features$financial_sentiment == "NaN"] <- 0
new_features$death_sentiment[new_features$death_sentiment == "NaN"] <- 0
new_features$pandemic_sentiment[new_features$pandemic_sentiment == "NaN"] <- 0
new_features$disease_sentiment[new_features$disease_sentiment == "NaN"] <- 0
new_features$illness_sentiment[new_features$illness_sentiment == "NaN"] <- 0
...
write_csv(new_features, path = 'sentiment_by_words_new_features.csv')
