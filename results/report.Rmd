...
From a policy perspective, greater impact and studies could be done to influence greater transparency$^{6}$ in reporting across the news companies. In order to make inform decisions on which news sources to read or be aware of the bias different sources might have.

# Data Description

## Data Sourcing

In order to choose major news sources to analyze, diversity of news sources was important. 
Therefore, we used the figure below created by the Pew Research Center. This figure shows where 
on the US political spectrum various news companies fall. This criteria was used to get a mix of sources across the conservative and liberal spectrum, while balancing limitations of computing resources. The following news sources were used to procure a dataset.  
...
The time frame used for this analysis was between January/1/2020 - April/9/2021.

A total of 5,360 articles were called from the API (20 articles per week and news source, for 67 weeks). The API call returned 3,454 records for article-related data. Below are some of the key fields from the API call, with an example:
...
BBC news articles used different HTML structures based on each section. Hence, the need to use keyword function in “Gnews API” to extract articles only in “news” tab. This allowed to scrap all body text. However, it also scrapped advertisement and unnecessary information. As a result, there was a need to remove irrelevant lines and words to prevent these features from affecting the analysis.
...
The original plan was to extract articles from a conservative newspaper, such as Fox news. However, Fox news updated their Terms of Use which strictly forbidden to use *“…any automated means, including "robots," "spiders," or "offline readers"…”*. Thus, it was decided to scrape The Wall Street Journal instead. One downside of using The Wall Street Journal was that it is a subscribe-based website; and it only allows ordinary viewers to extract a fraction of the articles. This could potentially explain the small amount of average word count per article. Similarly to BBC, scraped body text contained irrelevant information and it was removed with further cleaning process.

# Exploratory Data Analysis

To better understand the data, sentiment analysis utilizing various lexicons were 
explored. And it was conclude that the most informative one for this analysis was AFFIN lexicon.
Additionally, key visualizations of the data were illustrated:
...
The analysis was made with three sets of features in order to evaluate the bias and  different patterns 
accross all the news sources. The purpose of each set of features was to explain a different dimension in 
which the news sources might be differentiated. The three dimensions modeled were: sentiment based on key words, 
vocabulary previously studied in other papers, and topic content. 

The analysis was split in two. First, the bias accross sources was measured with a chi-squared test in each 
of the explained dimension. Second, the patterns were analyzed through different classification algorithms.

## Feature Engineering

In order to create features to build classification models for predicting a 
news source based on text and measure bias accross sources, the below features were generated.
...
In order to evaluate existing literature on politization and polarization, features based on previous published papers were generated.

- First paper by Hart, Chinn and Soroka$^{8}$ introduced a dictionary, which assembled major words with the most relevant vocabularies such as Covid-19, Scientist, Republican and Democrat, to explore politicization and polarization in pandemic news in the U.S. newspapers from March to May 2020. 

- Second paper by Green et al$^{9}$ demonstrated an absolute difference in the proportion of words used by political party using Tweet data from January to March 2020.

-	Final research by Schaeffer$^{10}$ also shows the difference in views on various topics based on their political stands from January and Feb 2021.

- The new features computed the frequency of these list of words used in each article. Since these features are not extracted nor inferred from the collected article data but from a broader analysis, it can be expected that these features can enhance our model’s performance.
...
- Republican: "republican","gop", 'conservative', "trump", "pence", "mcconnell","white house","administration"

- Democrat: "democrat","liberal","progressive","pelosi" ,"schumer", "biden", "obama","newsom" , "whitmer", "cuomo" , "biden," ,"sanders"

- Republican Words: "coronavirus","china","businesses","realdonaltrump","relief","inittogether","small","together", "cares", "great"
...
A chi-square test was perform, in two of the previous dimensions (topic probability and average sentiment), as well as the word count, to measure the signifance 
of differences across news sources. The chi-square test for all three metrics suggest that there is indeed bias in article text across the news sources.
...
In order to analyze the difference in patterns across news sources, 5 algorithms were evaluated. Each algorithm, ran 4 models using different sets of 
features:

- **Topic Modeling features**: per section 5.1.3

- **Keyword features**: per section 5.1.1

- **Topic keyword features**: per section 5.1.2
...
that the article text varies significantly in some dimensions across news sources.

Of all the models we ran, the random forest using all features performed the best (90% accuracy, 
98% AUC). The models that performed least favorably were Naive Bayes and Support Vector Machine. 
The random forest model performed the best because it is a non-parametric model without strong 
assumptions. Random Forest is designed as a sequence of decision trees that can classify accuratly 
clasify "clusters" of characteristics. Since the news sources have inherent topics (i.e. Business in The Wall Street Journal) they are more likely to write about, 
but have some degree of overlap, the random forest can model these patterns more effiently vs. other models such as logistic regression.  
...
as opposed to multi-class problems. Even when it is possible to fit the models as binary predictions. 
It is time consuming and there are other factors that need to be taken into account, such as rebalance of the train samples.
...
It seems that, as other papers have suggested, there is some bias align in the news sources article. The bias in different dimensions generates patterns that can accurately predict the news sources of which a text is coming from. This information could potentially be used towards making more transparent the bias across news sources. It is also a foundation towards modeling sentiment across different topics to evaluate the impact in health issued. 
...
-	Calculating sentiment scores from news articles to predict the publisher may not be an effective mechanism. Our research question hypothesized that the average sentiment of articles from each newspaper on a single phenomenon is not equal. It is true that each newspaper has its own perspectives in general. However, it is difficult to detect and to measure these patterns based on each article’s sentiment; news articles are mostly written with an objective and logical tone. It would be more comprehensible if we focus on the headline of the articles$^{11}$ or data from social network service such as Facebook or Twitter as they are more likely to include emotional words. In this paper, the researchers used machine learning models that we used to predict three major sentiments based on Twitter data with a higher accuracy.$^{12}$  
